...
public class WeightedPredictorController {
	...
	public void weightedPrediction() throws Exception {
		...
		for (int k = 0; k < valueRF.length; k++) {
			...
			if(splitData[0].equalsIgnoreCase("0"))
				splitData[0] = ""+0.001;
			...
			//"APPLE", "CISCO", "FACEBOOK", "GOOGLE", "IBM", "INTUIT", "MICROSOFT", "VARIAN"
			
			String compUrl = "";
			if(compNameDT[k].equalsIgnoreCase("APPLE"))
			so.setShareUrl("graph-appl.html");
			else if(compNameDT[k].equalsIgnoreCase("CISCO"))
				so.setShareUrl("graph-cisco.html");
			else if(compNameDT[k].equalsIgnoreCase("FACEBOOK"))
				so.setShareUrl("graph-fb.html");
			else if(compNameDT[k].equalsIgnoreCase("IBM"))
				so.setShareUrl("graph-ibm.html");
			else if(compNameDT[k].equalsIgnoreCase("INTUIT"))
				so.setShareUrl("graph-intu.html");
			else if(compNameDT[k].equalsIgnoreCase("MICROSOFT"))
				so.setShareUrl("graph-msft.html");
			else if(compNameDT[k].equalsIgnoreCase("VARIAN"))
				so.setShareUrl("raph-varian.html");
			else if(compNameDT[k].equalsIgnoreCase("GOOGLE"))
				so.setShareUrl("graph-goog.html");
